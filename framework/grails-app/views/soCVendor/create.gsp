...
<html>
	...
	<body>
		...
		<div id="create-soCVendor" class="content scaffold-create" role="main">
			...
			<g:hasErrors bean="${soCVendorInstance}">
			<ul class="errors" role="alert">
				<g:eachError bean="${soCVendorInstance}" var="error">
		...
		<div id="list-soCVendor" class="content scaffold-list" role="main">
			...
			<table style="width:70%; align: left;">
				...
				<tbody>
				<% int count = 0; %> 
				<g:each in="${soCVendorInstanceList}" status="i" var="soCVendorInstance">
					<g:hiddenField id="listCount" name="listCount" value="${count}"/>
					<% count++ %>
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">					
						<td style="text-align : center;" >						
						<g:checkBox name="chkbox${count}" class ="checkbox" id ="${soCVendorInstance?.id}" value="${false}"  checked = "false"  onclick ="checkBoxClicked(this)" /> 
						<g:hiddenField id="idas" name="id${count}" value="${soCVendorInstance?.id}" />
						</td>
						<td style="text-align : center;"><a href = '#' id="${soCVendorInstance.id}"  onclick ="populateSoCVendorField(this)">
							${fieldValue(bean: soCVendorInstance, field: "name")}</a></td>					
					</tr>
				</g:each>
				</tbody>
			</table>
