...
<xml>
  ...
  <name>DS_setSPDIF_SURROUNDFormat_8005_3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Device Setting –  Get and set stereo mode to SURROUND in SPDIF.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DS_setSPDIF_SURROUNDFormat_8005_3');
...
if "SUCCESS" in loadmodulestatus.upper():
		...
		if stereomode in stereomodedetails:
				...
				print "FAILURE: Application Failed to set and get the %s mode to SPDIF"%stereomode;
                	        tdkTestObj.setResultStatus("FAILURE");
             	   	else:
                        	if stereomode in stereomodedetails:
                                	tdkTestObj.setResultStatus("SUCCESS");
	                                print "SUCCESS: %s Mode set for SPDIF"%stereomode;
        	                else:
                	                tdkTestObj.setResultStatus("FAILURE");
					print "FAILURE: %s Mode not set for SPDIF"%stereomode;
		else:
			tdkTestObj.setResultStatus("FAILURE");
                        print "FAILURE: %s Mode not supported for SPDIF"%stereomode;
