...
<xml>
  <id/>
  <version>1</version>
  <name>Bluetooth_StartAudio_StreamingOut_Device_Properties</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_GetDeviceProperties</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check the connected device properties are updated when the audio streaming started</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_58</test_case_id>
    <test_objective>To check the connected device properties are updated when the audio streaming started</test_objective>
...
12.Get the device properties of the bluetooth emulator
13.Stop the audio streaming out and check audio streaming out is stopped or not then unpair the bluetooth emulator with the DUT</automation_approch>
...
Checkpoint 4 After started audio streaming out check the bluetooth emulator name in the connectedevices list of DUT.After that check connected device propery value is 1 or not
...
Checkpoint 6 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_StartAudio_StreamingOut_Device_Properties</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_StartAudio_StreamingOut_Device_Properties');
                                               ...
                                               if str(bluetoothlib.deviceName) in connectedDeviceNameList :
                                                   ...
                                                   if actualresult == expectedresult:
                                                       ...
                                                       if "1" in deviceProperties['connected']:
                                                           tdkTestObj.setResultStatus("SUCCESS");
                                                           print"Connected Device properties has valid value"
                                                       else:
                                                           print"Connected Device properties has Invalid value"
                                                           tdkTestObj.setResultStatus("FAILURE");
                                                   else:
