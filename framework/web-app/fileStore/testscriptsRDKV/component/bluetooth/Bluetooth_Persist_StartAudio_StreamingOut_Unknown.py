...
<xml>
  <id/>
  <version>2</version>
  <name>Bluetooth_Persist_StartAudio_StreamingOut_Unknown</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_StartAudioStreamingOut</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether connected devices details are persisting or not when the device type is Unknown</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_38</test_case_id>
    <test_objective>To check whether connected devices details are persisting or not when the device type is Unknown</test_objective>
...
4.Start the device discovery with  the device type as unknown  in DUT
5.Stop the device discovery  with the device type as unknown after 30 seconds
...
9.Start the audio out streaming in the device with device type unknown
...
Checkpoint 6 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Persist_StartAudio_StreamingOut_Unknown</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Persist_StartAudio_StreamingOut_Unknown');
                       ...
                       if actualresult == expectedresult:
                           ...
                           if actualresult == expectedresult:
                               ...
                               if str(bluetoothlib.deviceName) in pairedDeviceNameList :
                                   ...
                                   if actualresult == expectedresult:
                                       tdkTestObj.setResultStatus("SUCCESS");
                                       print "Bluetooth_StartAudioStreamingOut call is SUCCESS with device type as unknown"
                                       ...
                                       if audioStreamingStartStatus == '1':
                                           tdkTestObj.setResultStatus("SUCCESS");
                                           print "Audio Streaming Out Started Successfully with device type as unknown"
                                           #Reboot the STB
                                           ...
                                           tdkTestObj.setResultStatus("FAILURE"); 
                                           print "Audio Streaming Out is NOT Started with device type as unknown"
                                   else: 
                                       tdkTestObj.setResultStatus("FAILURE");
