...
<xml>
  ...
  <status>FREE</status>
  <synopsis>To check whether undiscoverable  devices are removed from the list once it is available in the discovered devices list when the device type is LE</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_31</test_case_id>
    <test_objective>To check whether undiscoverable  devices are removed from the list once it is available in the discovered devices list when the device type is LE</test_objective>
...
6.Stop the device discovery with the device type as LE after 30 seconds
...
9.Start the device discovery with the device type as LE in DUT
10.Stop the device discovery with the device type as LE after 30 seconds
