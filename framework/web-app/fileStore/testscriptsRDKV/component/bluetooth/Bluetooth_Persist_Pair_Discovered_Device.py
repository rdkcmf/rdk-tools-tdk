...
<xml>
  <id/>
  <version>1</version>
  <name>Bluetooth_Persist_Pair_Discovered_Device</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_PairDevice</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether Bluetooth pairing persists even after reboot provided discoverable status is on</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_15</test_case_id>
    <test_objective>To check whether Bluetooth pairing persists even after rebooting the DUT provided discoverable status is on in bluetooth emulator</test_objective>
...
9.Reboot the box
10.Check the paired devices list in DUT and confirm the bluetooth emulator adapter name is there in the list
11.Unpair the bluetooth emulator
12.Check the paired devices list in DUT and confirm the bluetooth emulator adapter name is NOT there in the list</automation_approch>
...
Checkpoint 3 After pairing and reboot the bluetooth emulator name should be there in the paired devices list of DUT
Checkpoint 4 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothstub.so.0</test_stub_interface>
    <test_script>Bluetooth_Persist_Pair_Discovered_Device</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Persist_Pair_Discovered_Device');
                                       ...
                                       if actualresult == expectedresult:
                                           ...
                                           if str(bluetoothlib.deviceName) in pairedDeviceNameList :
                                               ...
                                               if actualresult == expectedresult:
                                                   ...
                                                   actualresult = tdkTestObj.getResult();
                                                   discoveredDevicesList = tdkTestObj.getResultDetails();
                                                   pairedDevicesList = discoveredDevicesList.split(';')[:-1]
                                                   print "Paired Devices List" , pairedDevicesList
                                                   if actualresult == expectedresult:
                                                       tdkTestObj.setResultStatus("SUCCESS");
                                                       print "Bluetooth_GetPairedDevices call is SUCCESS"
                                               ...
                                               tdkTestObj.setResultStatus("FAILURE");
                                               print "Client device is NOT automatically paired with DUT After Reboot"
                                       else:
                                           tdkTestObj.setResultStatus("FAILURE");
