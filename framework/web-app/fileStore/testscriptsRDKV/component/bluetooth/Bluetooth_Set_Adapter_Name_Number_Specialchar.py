...
<xml>
  <id/>
  <version>1</version>
  <name>Bluetooth_Set_Adapter_Name_Number_Specialchar</name>
  <primitive_test_id/>
...
3.Bluetooth agent will invoke the api BTRMGR_SetAdapterName with adapter name as "12#4abc@"
4.Check whether the new adapter name "12#4abc@" is set or not
5.Set the previous adapter name again</automation_approch>
    <except_output>Checkpoint 1.Verify the API call return value
Checkpoint 2.The new adapter name "12#4abc@" should set successfully .</except_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothstub.so.0</test_stub_interface>
    <test_script>Bluetooth_Set_Adapter_Name_Number_Specialchar</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Set_Adapter_Name_Number_Specialchar');
