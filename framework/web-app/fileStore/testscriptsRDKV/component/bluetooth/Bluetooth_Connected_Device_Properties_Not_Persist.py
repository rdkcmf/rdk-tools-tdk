...
<xml>
  <id/>
  <version>1</version>
  <name>Bluetooth_Connected_Device_Properties_Not_Persist</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_GetDeviceProperties</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether connected device details are  persists or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_59</test_case_id>
    <test_objective>To check whether connected device details are  persists or not</test_objective>
...
11. Reboot and check the Get the connected device properties
...
Checkpoint 5 Reboot and Check the connected device properties values . value of connected should be equal to 0
...
Checkpoint 7 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Connected_Device_Properties_Not_Persist</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Connected_Device_Properties_Not_Persist');
                                       ...
                                       if actualresult == expectedresult:
                                           ...
                                           if "1" in deviceProperties['connected']:
                                               ...
                                               if actualresult == expectedresult:
                                                   ...
                                                   json_acceptable_string = deviceProperties.replace("'", "\"")
                                                   deviceProperties =json.loads(json_acceptable_string)
                                                   if "1" not in deviceProperties['connected']:
                                                       tdkTestObj.setResultStatus("SUCCESS");
                                                       print "Connected device details should not persist"
                                                   else:
                                                       tdkTestObj.setResultStatus("FAILURE");
                                                       print "Connected device details persisted"
                                               else:
                                                   tdkTestObj.setResultStatus("FAILURE");
