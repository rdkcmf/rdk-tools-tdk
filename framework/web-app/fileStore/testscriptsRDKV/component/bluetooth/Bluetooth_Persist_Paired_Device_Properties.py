...
<xml>
  <id/>
  <version>2</version>
  <name>Bluetooth_Persist_Paired_Device_Properties</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_GetDeviceProperties</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether the paired device properties persists or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_53</test_case_id>
    <test_objective>To check whether the paired device properties persists or not</test_objective>
...
10. Reboot the DUt and Get the paired device properties
...
Checkpoint 5 After reboot also Check the paired device properties values . value of paired should be equal to 1</except_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothstub.so.0</test_stub_interface>
    <test_script>Bluetooth_Persist_Paired_Device_Properties</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Persist_Paired_Device_Properties');
                               ...
                               if str(bluetoothlib.deviceName) in pairedDeviceNameList :
                                   ...
                                   if actualresult == expectedresult:
                                       ...
                                       if "1" in deviceProperties['paired']:
                                           ...
                                           sleep(60);
                                           print "Get the paired device properties after reboot"
                                           tdkTestObj = bluetoothObj.createTestStep('Bluetooth_GetDeviceProperties');
                                           tdkTestObj.addParameter("devicehandle",handleNumber);
                                           #Execute the test case in STB
                                           tdkTestObj.executeTestCase(expectedresult);
                                           ...
                                           if actualresult == expectedresult:
                                               ...
                                               if "1" in deviceProperties['paired']:
                                                   tdkTestObj.setResultStatus("SUCCESS");
                                                   print"Paired device property value has persisted"
                                               else:
                                                   print"Paired device property value is NOT persisted"
                                                   tdkTestObj.setResultStatus("FAILURE");
                                       else:
