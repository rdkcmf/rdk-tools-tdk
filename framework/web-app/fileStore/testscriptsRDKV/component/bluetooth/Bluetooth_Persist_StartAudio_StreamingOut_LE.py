...
<xml>
  <id/>
  <version>2</version>
  <name>Bluetooth_Persist_StartAudio_StreamingOut_LE</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_StartAudioStreamingOut</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether connected devices details are persisting or not when the device type is LE</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_37</test_case_id>
    <test_objective>To check whether connected devices details are persisting or not when the device type is LE</test_objective>
...
9.Start the audio out streaming in the device with device type audio LE
...
Checkpoint 6 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Persist_StartAudio_StreamingOut_LE</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Persist_StartAudio_StreamingOut_LE');
