...
<xml>
  <id/>
  <version>2</version>
  <name>Bluetooth_Persist_StartAudio_StreamingOut_Audio_Input</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_StartAudioStreamingOut</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether audio streaming out device is persisting or not when the device type is audio input</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_36</test_case_id>
    <test_objective>To check whether audio streaming out device is persisting or not when the device type is audio input</test_objective>
...
4.Start the device discovery with  the device type as audio input  in DUT
5.Stop the device discovery  with the device type as audio input after 30 seconds
...
9.Start the audio out streaming in the device with device type audio input
...
11.Stop the audio streaming out and check audio streaming out is stopped or not then unpair the bluetooth emulator with the DUT</automation_approch>
...
Checkpoint 4 After started audio streaming out and reboot the DUT then check the bluetooth emulator name in the connectedevices list of DUT
...
Checkpoint 6 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Persist_StartAudio_StreamingOut_Audio_Input</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Persist_StartAudio_StreamingOut_Audio_Input');
                       ...
                       if actualresult == expectedresult:
                           ...
                           if actualresult == expectedresult:
                               ...
                               if str(bluetoothlib.deviceName) in pairedDeviceNameList :
                                   ...
                                   if actualresult == expectedresult:
                                       tdkTestObj.setResultStatus("SUCCESS");
                                       print "Bluetooth_StartAudioStreamingOut call is SUCCESS with device type as audio input"
                                       ...
                                       if audioStreamingStartStatus == '1':
                                           tdkTestObj.setResultStatus("SUCCESS");
                                           print "Audio Streaming Out Started Successfully with device type as audio input"
                                           #Reboot the STB
                                               ...
                                               if(audioStreamingStopStatus == '0'):
                                                   ...
                                                   if actualresult == expectedresult:
                                                       ...
                                                       if str(bluetoothlib.deviceName) not in connectedDeviceNameList :
                                                           tdkTestObj.setResultStatus("SUCCESS");
                                                           print "Client device is successfully disconnected from DUT after stopping the audio streaming out"
                                                       else:
                                                           tdkTestObj.setResultStatus("FAILURE");
                                                           print "Client device is NOT disconnected from DUT after stopping the audio streaming out"
                                                   else:
                                                       tdkTestObj.setResultStatus("FAILURE");
                                                       print "Bluetooth_GetConnectedDevices call is FAILURE"
                                               else: 
                                           ...
                                           tdkTestObj.setResultStatus("FAILURE"); 
                                           print "Audio Streaming Out is NOT Started with device type as audio input"
                                   else: 
                                       tdkTestObj.setResultStatus("FAILURE");
