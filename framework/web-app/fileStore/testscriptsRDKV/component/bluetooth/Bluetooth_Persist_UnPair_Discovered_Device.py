...
<xml>
  <id/>
  <version>1</version>
  <name>Bluetooth_Persist_UnPair_Discovered_Device</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_UnpairDevice</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether unpaired devices details persists or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_18</test_case_id>
    <test_objective>To check whether unpaired devices details persists or not</test_objective>
...
9.Unpair the bluetooth emulator and reboot the box
10.Check the paired devices list in DUT and confirm the bluetooth emulator adapter name is NOT there in the list</automation_approch>
...
Checkpoint 4 After unpair and reboot , the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothstub.so.0</test_stub_interface>
    <test_script>Bluetooth_Persist_UnPair_Discovered_Device</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Persist_UnPair_Discovered_Device');
                                           ...
                                           if str(bluetoothlib.deviceName) not in pairedDeviceNameList :
                                               ...
                                               if "FAILURE" in output:
                                                   ...
                                                   if actualresult == expectedresult:
                                                       ...
                                                       if str(bluetoothlib.deviceName) not in pairedDeviceNameList :
                                                           tdkTestObj.setResultStatus("SUCCESS");
                                                           print "Unpaired devices details are persisted after reboot" 
                                                       else:
                                                           tdkTestObj.setResultStatus("FAILURE");
                                                           print "Unpaired devices details are NOT persisted after reboot"
                                                   else:
                                                       tdkTestObj.setResultStatus("FAILURE");
