...
<xml>
  <id/>
  <version>2</version>
  <name>Bluetooth_StopAudio_StreamingOut_Not_Persist_Audio_Output</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_StopAudioStreamingOut</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether audio streaming out device details are NOT persisting with the device type as  audio output when the audio stream out is stopped before reboot</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_44</test_case_id>
    <test_objective>To check whether audio streaming out device details are NOT persisting with the device type as  audio output when the audio stream out is stopped before reboot</test_objective>
...
Checkpoint 5 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_StopAudio_StreamingOut_Not_Persist_Audio_Output</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_StopAudio_StreamingOut_Not_Persist_Audio_Output');
