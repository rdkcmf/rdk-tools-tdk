...
<xml>
  <id/>
  <version>1</version>
  <name>Bluetooth_Accept_External_Pair_Request</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_StartDeviceDiscovery</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether the device is getting paired when the pair request came from external device</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_54</test_case_id>
    <test_objective>To check whether the DUT is getting paired when the pair request came from external device</test_objective>
...
6.Unpair the bluetooth emulator
</automation_approch>
    <except_output>Checkpoint 1.After pairing the bluetooth emulator name should be there in the paired devices list of DUT
Checkpoint 2 After Unpair the bluetooth emulator  should be removed from the paired devices list of DUT</except_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothstub.so.0</test_stub_interface>
    <test_script>Bluetooth_Accept_External_Pair_Request</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Accept_External_Pair_Request');
