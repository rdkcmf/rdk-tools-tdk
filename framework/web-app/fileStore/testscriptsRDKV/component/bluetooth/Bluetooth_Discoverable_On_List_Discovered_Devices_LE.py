...
<xml>
  <id/>
  <version>5</version>
  <name>Bluetooth_Discoverable_On_List_Discovered_Devices_LE</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_GetDiscoveredDevices</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether discoverable  devices are listed in the discovered devices list when the decvice type is LE</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_34</test_case_id>
    <test_objective>To check whether discoverable  devices are listed in the discovered devices list when the decvice type is LE</test_objective>
...
5.Start the device discovery in DUT with LE as param
6.Stop the device discovery with  the decvice type as LE after 30 seconds
...
Checkpoint 2.The bluetooth emulator name should be there in the discovered devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Discoverable_On_List_Discovered_Devices_LE</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Discoverable_On_List_Discovered_Devices_LE');
