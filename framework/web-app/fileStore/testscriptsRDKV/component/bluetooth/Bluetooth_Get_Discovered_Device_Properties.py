...
<xml>
  <id/>
  <version>1</version>
  <name>Bluetooth_Get_Discovered_Device_Properties</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_20</test_case_id>
    <test_objective>To check the device properties of a discovered device</test_objective>
...
Bluetooth_GetDiscoveredDevices
...
8.Get the device properties of the bluetooth emulator</automation_approch>
...
Checkpoint 3 Compare the device properties values</except_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothstub.so.0</test_stub_interface>
    <test_script>Bluetooth_Get_Discovered_Device_Properties</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Get_Discovered_Device_Properties');
               ...
               if actualresult == expectedresult:
                   ...
                   if str(bluetoothlib.deviceName) in deviceNameList :
                       ...
                       if actualresult == expectedresult:
                           ...
                           if str(bluetoothlib.deviceName) in deviceProperties['name']  and handleNumber in deviceProperties['handle']:
                               tdkTestObj.setResultStatus("SUCCESS");
                               print("Device properties retrieved successfully")
                           else:
                               print("Device properties NOT retrieved successfully")
                               tdkTestObj.setResultStatus("FAILURE");
                       else:
                           tdkTestObj.setResultStatus("FAILURE");
