...
<xml>
  ...
  <name>Bluetooth_Discoverable_On_List_Discovered_Devices_Default_Input</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether discoverable  devices are listed in the discovered devices list with Default input</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_49</test_case_id>
    <test_objective>To check whether discoverable  devices are listed in the discovered devices list of DUT with Default input</test_objective>
...
Checkpoint 2.The bluetooth emulator name should be there in the discovered devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Discoverable_On_List_Discovered_Devices_Default_Input</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Discoverable_On_List_Discovered_Devices_Default_Input');
...
if "SUCCESS" in bluetoothLoadStatus.upper():
   ...
   if "FAILURE" in output:
       ...
       tdkTestObj.addParameter("devicetype",100)
       #Execute the test case in STB
       tdkTestObj.executeTestCase(expectedresult);
       actualresult = tdkTestObj.getResult();
       if actualresult == expectedresult:
