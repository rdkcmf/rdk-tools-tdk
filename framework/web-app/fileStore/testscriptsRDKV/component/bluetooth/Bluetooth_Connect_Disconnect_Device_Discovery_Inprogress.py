...
<xml>
  ...
  <name>Bluetooth_Connect_Disconnect_Device_Discovery_Inprogress</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To connect and disconnect with a device when the device discovery is in progress</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_24</test_case_id>
    <test_objective>To connect and disconnect with a device when the device discovery is in progress</test_objective>
...
7.Pair with the bluetooth emulator when the device discovery is in progress
...
Checkpoint 6 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Connect_Disconnect_Device_Discovery_Inprogress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Connect_Disconnect_Device_Discovery_Inprogress');
