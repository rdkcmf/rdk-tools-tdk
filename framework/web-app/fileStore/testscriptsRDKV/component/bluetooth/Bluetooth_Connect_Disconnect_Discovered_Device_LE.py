...
<xml>
  <id/>
  <version>3</version>
  <name>Bluetooth_Connect_Disconnect_Discovered_Device_LE</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_ConnectToDevice</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To connect and disconnect with a discovered device when the device type is LE</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_28</test_case_id>
    <test_objective>To connect and disconnect with a discovered device when the device type is LE</test_objective>
...
4.Start the device discovery in DUT with device type as LE
5.Stop the device discovery  with device type as LE after 30 seconds
...
9.Connect the the device with LE as the device type
...
Checkpoint 6 After unpair, the bluetooth emulator name should NOT be there in the paired devices list of DUT</except_output>
    ...
    <test_script>Bluetooth_Connect_Disconnect_Discovered_Device_LE</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Connect_Disconnect_Discovered_Device_LE');
