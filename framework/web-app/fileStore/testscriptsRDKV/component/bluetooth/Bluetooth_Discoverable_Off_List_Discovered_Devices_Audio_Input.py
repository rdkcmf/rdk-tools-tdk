...
<xml>
  <id/>
  <version>3</version>
  <name>Bluetooth_Discoverable_Off_List_Discovered_Devices_Audio_Input</name>
  <primitive_test_id/>
  <primitive_test_name>Bluetooth_StartDeviceDiscovery</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether undiscoverable  devices are removed from the list once it is available in the discovered devices list when the device type is audio input</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>To check whether undiscoverable  devices are removed from the list once it is available in the discovered devices list when the device type is audio input</test_objective>
...
5.Start the device discovery in DUT with Audio_Input as input param
6.Stop the device discovery with the device type as audio input after 30 seconds
...
9.Start the device discovery with the device type as audio input in DUT
10.Stop the device discovery with the device type as audio input after 30 seconds
...
Checkpoint 3 The bluetooth emulator name should NOT be there in the discovered devices list of DUT after the discoverable status of bluetooth emulator turned OFF</except_output>
    ...
    <test_script>Bluetooth_Discoverable_Off_List_Discovered_Devices_Audio_Input</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
bluetoothObj.configureTestCase(ip,port,'Bluetooth_Discoverable_Off_List_Discovered_Devices_Audio_Input');
