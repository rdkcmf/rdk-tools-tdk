...
<xml>
  ...
  <name>DSHal_EnableVideoPort_UnSupportedPortType</name>
  ...
  <test_cases>
    ...
    <test_objective>To check if the video port enable status is set for unsupported port type</test_objective>
...
4. TM checks if the API call fails and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_EnableVideoPort_UnSupportedPortType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_EnableVideoPort_UnSupportedPortType');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            print "Video port enable status not set for unsupported port type which is expected";
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print "Video port enable status set for unsupported port type which is not expected";
    else:
        tdkTestObj.setResultStatus("FAILURE");
