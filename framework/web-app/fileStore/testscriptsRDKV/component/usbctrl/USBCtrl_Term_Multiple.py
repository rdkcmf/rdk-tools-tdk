...
<xml>
  ...
  <name>USBCtrl_Term_Multiple</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if multiple termination is possible for usbctrl</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_USBCtrl_04</test_case_id>
    <test_objective>To check if multiple termination is possible for usbctrl</test_objective>
...
2. USBCtrl_Agent will invoke rusbCtrl_term API to to initialize USBCtrl
...
Checkpoint 3.Check the result is not  0 on second API call</except_output>
    ...
    <test_script>USBCtrl_Term_Multiple</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'USBCtrl_Term_Multiple');
...
if "SUCCESS" in result.upper():
	...
	if "SUCCESS" in result:
		...
		if expectedresult in result:
			...
			print "Multiple termination Failed"
		else:
			tdkTestObj.setResultStatus("FAILURE");
			print "Multiple termination SUCCESS"
