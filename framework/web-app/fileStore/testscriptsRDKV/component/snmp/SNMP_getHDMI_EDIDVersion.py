...
<xml>
  ...
  <name>SNMP_getHDMI_EDIDVersion</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test gets the HDMI EDID version</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_SNMP_02</test_case_id>
    <test_objective>To get HDMI EDID version using snmpwalk command
</test_objective>
...
SnmpVersion : v2c
OID : 1.3.6.1.4.1.4491.2.3.1.1.1.2.4.1.1.25"
</input_parameters>
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager"
    ...
    <test_script>SNMP_getHDMI_EDIDVersion</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SNMP_getHDMI_EDIDVersion');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if "0.0" not in actResponse:
        ...
        print "TEST STEP 1:Execute snmpwalk to get the HDMI EDID Version";
        print "EXPECTED RESULT 1: snmpwalk should get the HDMI EDID Version";
        print "ACTUAL RESULT 1: %s" %actResponse;
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : HDMI EDID Version = %s" %actResponse ;
    else:
        ...
        print "EXPECTED RESULT 1: snmpwalk should get the HDMI EDID Version";
        print "ACTUAL RESULT 1: %s" %actResponse;
        print "[TEST EXECUTION RESULT] : EDID not available";
