...
<xml>
  ...
  <name>SNMP_getHDMI_MaxDevCount</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test gets the max number of HDMI devices connected</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_SNMP_10</test_case_id>
    <test_objective>To get max number of HDMI device connected using snmpwalk command
</test_objective>
...
SnmpVersion : v2c
OID : 1.3.6.1.4.1.4491.2.3.1.1.1.2.4.1.1.23"
</input_parameters>
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager"
    ...
    <test_script>SNMP_getHDMI_MaxDevCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SNMP_getHDMI_MaxDevCount');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if "SNMPv2-SMI" in actResponse:
        ...
        if int(devCount)>=0:
                ...
                print "[TEST EXECUTION RESULT] : SUCCESS :HDMIMaxDeviceCount =%s" %devCount ;
        else:
                ...
                print "EXPECTED RESULT 1: snmpwalk should get the maximum number of HDMI devices this device can support";
                print "ACTUAL RESULT 1: %s" %actResponse;
                print "[TEST EXECUTION RESULT] : FAILURE : Cannot connect HDMI devices";
