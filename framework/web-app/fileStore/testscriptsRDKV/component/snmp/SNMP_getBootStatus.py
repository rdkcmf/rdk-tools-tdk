...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test gets the host boot status</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_SNMP_01</test_case_id>
    <test_objective>To get boot status of the host using snmpget command</test_objective>
...
SnmpVersion : v2c
OID : 1.3.6.1.4.1.4491.2.3.1.1.4.5.6"</input_parameters>
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager"</except_output>
    ...
    <test_script>SNMP_getBootStatus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SNMP_getBootStatus');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    snmpResponse =snmplib.SnmpExecuteCmd("snmpwalk",commString, "-v 2c", ".1.3.6.1.4.1.4491.2.3.1.1.4.5.6", ip);
    actResponse =snmpResponse.split('=')[1];

    #Logic for verification will be done in the next iteration
    if "1" in actResponse:
        tdkTestObj.setResultStatus("SUCCESS");
        print "TEST STEP 1:Execute snmpwalk for host boot status";
        print "EXPECTED RESULT 1: snmpwalk should get the host boot status";
        print "ACTUAL RESULT 1: %s" %actResponse;
        print "Host boot status :completedSuccessfully";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : SUCCESS :completedSuccessfully" ;
    elif "2" in actResponse:
        tdkTestObj.setResultStatus("SUCCESS");
        print "TEST STEP 1:Execute snmpwalk for host boot status";
        print "EXPECTED RESULT 1: snmpwalk should get the host boot status";
        print "ACTUAL RESULT 1: %s" %actResponse;
        print "Host boot status :completeWithErrors";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : SUCCESS :completeWithErrors" ;
    elif "3" in actResponse:
        tdkTestObj.setResultStatus("SUCCESS");
        print "TEST STEP 1:Execute snmpwalk for host boot status";
        print "EXPECTED RESULT 1: snmpwalk should get the host boot status";
        print "ACTUAL RESULT 1: %s" %actResponse;
        print "Host boot status :inProgressWithCodeDownload";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : SUCCESS :inProgressWithCodeDownload" ;
    elif "4" in actResponse:
        tdkTestObj.setResultStatus("SUCCESS");
        print "TEST STEP 1:Execute snmpwalk for host boot status";
        print "EXPECTED RESULT 1: snmpwalk should get the host boot status";
        print "ACTUAL RESULT 1: %s" %actResponse;
        print "Host boot status :inProgressNoCodeDownload";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : SUCCESS :inProgressNoCodeDownload" ;
    elif "5" in actResponse:
        tdkTestObj.setResultStatus("SUCCESS");
        print "TEST STEP 1:Execute snmpwalk for host boot status";
        print "EXPECTED RESULT 1: snmpwalk should get the host boot status";
        print "ACTUAL RESULT 1: %s" %actResponse;
        print "Host boot status :inProgressAwaitingMonitorApp";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : SUCCESS :inProgressAwaitingMonitorApp" ;
    else:
        ...
        print "EXPECTED RESULT 1: snmpwalk should get the host boot status";
        print "ACTUAL RESULT 1: %s" %actResponse;
        print "[TEST EXECUTION RESULT] : FAILURE : INVALID";
