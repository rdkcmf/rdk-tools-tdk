...
<xml>
  ...
  <name>SNMP_getEthernetEnabledStatus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test gets the ethernet enable status of the host</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_SNMP_09</test_case_id>
    <test_objective>To get ethernet enabled status of the host using snmpwalk command
</test_objective>
...
SnmpVersion : v2c
OID : 1.3.6.1.4.1.17270.9225.1.4.1.18"
</input_parameters>
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager"
    ...
    <test_script>SNMP_getEthernetEnabledStatus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SNMP_getEthernetEnabledStatus');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    actResponse =snmplib.SnmpExecuteCmd("snmpwalk",commString, "-v 2c", "1.3.6.1.4.1.17270.9225.1.4.1.18", ip);
  
    #Logic for verification will be done in the next iteration
    if "TRUE" or "FALSE" in actResponse:
        tdkTestObj.setResultStatus("SUCCESS");
        print "TEST STEP 1:Execute snmpwalk to get Ethernet enabled status";
        print "EXPECTED RESULT 1: snmpwalk should get Ethernet enabled status";
        print "ACTUAL RESULT 1: %s" %actResponse;
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : SUCCESS :Ethernet enable status: %s" %actResponse;
    else:
