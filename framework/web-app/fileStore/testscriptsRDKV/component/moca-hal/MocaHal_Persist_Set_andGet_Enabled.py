...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Persist_Set_andGet_Enabled</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_SetEnabled</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether moca enable status is persisting or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_12</test_case_id>
    <test_objective>To check whether moca enable status is persisting or not</test_objective>
...
4.Reboot the STB and confirm enable status is persisting after moca handle destroy and reinitialize
...
Checkpoint 2 Verify the enable status is true before and after moca handle destroy and reinitialize</except_output>
    <priority>High</priority>
    <test_stub_interface>libmocahalstub.so</test_stub_interface>
    <test_script>MocaHal_Persist_Set_andGet_Enabled</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Persist_Set_andGet_Enabled');
...
if "SUCCESS" in mocaLoadStatus.upper():
   ...
   if "true" in enabled :
       ...
       print "Unloading and loading the moca hal module again to destroy the moca handle and reinitialize"
       mocahalObj.unloadModule("mocahal");
       mocahalObj = tdklib.TDKScriptingLibrary("mocahal","2.0");
       mocahalObj.configureTestCase(ip,port,'MocaHal_Persist_Set_andGet_Enabled');

       print "Check whether the moca enbaled value is persisting or not"
       tdkTestObj = mocahalObj.createTestStep('MocaHal_GetEnabled');
       #Execute the test case in STB
       tdkTestObj.executeTestCase(expectedresult);
       actualresult = tdkTestObj.getResult();
       print "RESULT: Getenabled After moca handle destroy and reinitialize : " , actualresult
       enabled = tdkTestObj.getResultDetails();
       print "DETAILS: GetEnabled After moca handle destroy and reinitialize : " , enabled

       if "true" in enabled :
           print "Moca enabled value persisted successfully"
           tdkTestObj.setResultStatus("SUCCESS")
       else:
           print "Moca enabled value is NOT persisted"
