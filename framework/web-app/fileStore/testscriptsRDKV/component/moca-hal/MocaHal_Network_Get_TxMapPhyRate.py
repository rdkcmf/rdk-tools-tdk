...
<xml>
  ...
  <name>MocaHal_Network_Get_TxMapPhyRate</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>MocaHal_GetTxMapPhyRate</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the PHY rate at which MAP packets are transmitted from the node</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_39</test_case_id>
    <test_objective>To get the PHY rate at which MAP packets are transmitted from the node</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_Network_GetTxMapPhyRate</api_or_interface_used>
...
2 .Moca hal agent will invoke the api   SoC_IMPL__RMH_Network_GetTxMapPhyRate
3. Verify that the TxMapPhyRate is retrieved
...
Checkpoint 2 Verify that theTxMapPhyRate  is retrieved</expected_output>
    <priority>High</priority>
    <test_stub_interface>libmocahalstub.so</test_stub_interface>
    <test_script>MocaHal_Network_Get_TxMapPhyRate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Network_Get_TxMapPhyRate');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    tdkTestObj = mocahalObj.createTestStep('MocaHal_GetTxMapPhyRate');
    #Execute the test case in STB
    tdkTestObj.executeTestCase(expectedresult);
    actualresult = tdkTestObj.getResult();
    print "RESULT: GetTxMapPhyRate : " , actualresult
    phyRate = tdkTestObj.getResultDetails();
    print "DETAILS: GetTxMapPhyRate : " , phyRate
    
    if expectedresult in actualresult and phyRate:
        tdkTestObj.setResultStatus("SUCCESS")
        print "TxMapPhyRate retrieved" , phyRate
    else:
        tdkTestObj.setResultStatus("FAILURE")
        print "TxMapPhyRate not retrieved"
