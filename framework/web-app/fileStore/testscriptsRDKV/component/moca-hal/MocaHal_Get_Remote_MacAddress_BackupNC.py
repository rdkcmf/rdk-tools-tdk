...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_Remote_MacAddress_BackupNC</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetMac</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the mac address of the backup network co-ordinator using RemoteNode_GetMac method</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_40</test_case_id>
    <test_objective>To get the mac address of the backup network co-ordinator using RemoteNode_GetMac method</test_objective>
...
3. Check the api call return value and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_Remote_MacAddress_BackupNC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_Remote_MacAddress_BackupNC');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    if expectedresult in actualresult and bkNcNodeId:
        ...
        if expectedresult in actualresult and macAddr:
            tdkTestObj.setResultStatus("SUCCESS")
            print "Backup NC MacAddress retrieved" , macAddr
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Backup NC MacAddress not retrieved" 
    else:
        tdkTestObj.setResultStatus("FAILURE")
