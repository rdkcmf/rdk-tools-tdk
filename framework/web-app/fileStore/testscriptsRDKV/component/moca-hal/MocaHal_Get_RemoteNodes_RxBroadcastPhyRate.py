...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_RemoteNodes_RxBroadcastPhyRate</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetRxBroadcastPhyRate</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the RxBroadcastPhyRate of the remote node</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_69</test_case_id>
    <test_objective>To get the RxBroadcastPhyRate of the remote node</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_RemoteNode_GetRxBroadcastPhyRate</api_or_interface_used>
...
[out]	response	The RxBroadcastPhyRate of the remote node.</input_parameters>
...
3. Moca hal agent will invoke the api SoC_IMPL__RMH_RemoteNode_GetRxBroadcastPhyRate with each node id
4. Check the api call return value and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_RemoteNodes_RxBroadcastPhyRate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_RemoteNodes_RxBroadcastPhyRate');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    if expectedresult in actualresult and remoteNodeList:
        ...
        for nodeId in remoteNodeList:
            #Prmitive test case to get remote node RxBroadcastPhyRate
            tdkTestObj = mocahalObj.createTestStep('MocaHal_RemoteNode_GetRxBroadcastPhyRate');
            tdkTestObj.addParameter("nodeId",int(nodeId));
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            print "RESULT: RemoteNode_GetRxBroadcastPhyRate : " , actualresult
            phyRate = tdkTestObj.getResultDetails();
            print "DETAILS: RemoteNode_GetRxBroadcastPhyRate : " , phyRate

            if expectedresult in actualresult and phyRate:
                tdkTestObj.setResultStatus("SUCCESS")
                print "RxBroadcastPhyRate %s retrieved for node id %d" %(phyRate, int(nodeId))
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "RxBroadcastPhyRate not retrieved for node id ", int(nodeId)
    else:
        tdkTestObj.setResultStatus("FAILURE")
