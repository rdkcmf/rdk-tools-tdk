...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_SelfNode_RxTotalPackets</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_GetRxTotalPackets</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the total number of packets received by this node</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_54</test_case_id>
    <test_objective>To get the total number of packets received by this node</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_Stats_GetRxTotalPackets</api_or_interface_used>
    <input_parameters>[in]	handle	The RMH handle as preturned by RMH_Initialize.
[out]	response	The number of packets received by the node</input_parameters>
    <automation_approch>1. TM loads the Moca hal agent via the test agent.
2. Using systemutil ExecuteCmd command get the rx packets using linux command
3. Moca hal agent will invoke the api SoC_IMPL__RMH_Stats_GetRxTotalPackets 
4. Check the value from mocahal interface is greater than or equal to the value from linux command and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_SelfNode_RxTotalPackets</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_SelfNode_RxTotalPackets');
sysObj.configureTestCase(ip,port,'MocaHal_Get_SelfNode_RxTotalPackets');
...
if "SUCCESS" in mocaLoadStatus.upper() and sysLoadStatus.upper():
    ...
    if expectedResult in actualResult and ifName:
        ...
        cmd = "cat /sys/class/net/" + ifName + "/statistics/rx_packets | tr -d '\n'";
        tdkTestObj.addParameter("command", cmd);
        tdkTestObj.executeTestCase("SUCCESS");
        actualResult = tdkTestObj.getResult();
        rxPackets = tdkTestObj.getResultDetails();
        print "RxPackets from linux command: ",rxPackets

        if expectedResult in actualResult and rxPackets:
            tdkTestObj.setResultStatus("SUCCESS")
            #Prmitive test case 
            tdkTestObj = mocahalObj.createTestStep('MocaHal_GetRxTotalPackets');
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            actualResult = tdkTestObj.getResult();
            print "RESULT: GetRxTotalPackets : " , actualResult
            rxTotalPackets = tdkTestObj.getResultDetails();
            print "DETAILS: GetRxTotalPackets : " , rxTotalPackets

            if expectedResult in actualResult and rxTotalPackets:
                print "RxTotalPackets retrieved: ", rxTotalPackets
                if rxTotalPackets >= rxPackets:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print "RxTotalPackets verified"
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print "RxTotalPackets not verified"
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "RxTotalPackets not not retrieved"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Execute command failed to get rx packets"
    else:
        tdkTestObj.setResultStatus("FAILURE")
