...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_NCMac_Address</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_GetNCMac</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the mac address of the network co-ordinator</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_37</test_case_id>
    <test_objective>To get the mac address of the network co-ordinator</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_Network_GetNCMac</api_or_interface_used>
...
2 .Moca hal agent will invoke the api SoC_IMPL__RMH_Network_GetNCMac
3. Check the api call return value and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_NCMac_Address</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_NCMac_Address');
...
if "SUCCESS" in mocaLoadStatus.upper():
   ...
   if "SUCCESS" in actualresult:
       ...
       print "MocaHal_GetNCMac API call is success"
   else:
       tdkTestObj.setResultStatus("FAILURE")
       print "MocaHal_GetNCMac API call is NOT success"
