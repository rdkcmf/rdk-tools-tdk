...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>MocaHal_Check_SelfNodeId_In_RemoteNodeIdsList</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_GetRemoteNodeIds</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that self node id is not present in list of remote node ids</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_35</test_case_id>
    <test_objective>To verify that self node id is not present in list of remote node ids</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_Network_GetRemoteNodeIds</api_or_interface_used>
...
3. Verify that the remote node id list is retrieved
4. Verify that the self node id is not present in the list and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1.Verify the API call return value
Checkpoint 2 Verify that the self node id is not present in the list </expected_output>
    <priority>High</priority>
    <test_stub_interface>libmocahalstub.so</test_stub_interface>
    <test_script>MocaHal_Check_SelfNodeId_In_RemoteNodeIdsList</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Check_SelfNodeId_In_RemoteNodeIdsList');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    if expectedresult in actualresult and remoteNodeList:
        ...
        if expectedresult in actualresult and selfNodeId:
            ...
            if selfNodeId not in remoteNodeList:
                tdkTestObj.setResultStatus("SUCCESS")
                print "Self node id is not present in remote node list" 
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "Self node id is present in remote node list" 
        else:
            tdkTestObj.setResultStatus("FAILURE")
