...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_SelfNode_TxTotalPackets</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_GetTxTotalPackets</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the total number of packets transmitted by this node</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_53</test_case_id>
    <test_objective>To get the total number of packets transmitted by this node</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_Stats_GetTxTotalPackets</api_or_interface_used>
    <input_parameters>[in]	handle	The RMH handle as preturned by RMH_Initialize.
[out]	response	The number of packets transmitted by the node</input_parameters>
    <automation_approch>1. TM loads the Moca hal agent via the test agent.
2. Using systemutil ExecuteCmd command get the tx packets using linux command
3. Moca hal agent will invoke the api SoC_IMPL__RMH_Stats_GetTxTotalPackets 
4. Check the value from mocahal interface is greater than or equal to the value from linux command and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_SelfNode_TxTotalPackets</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_SelfNode_TxTotalPackets');
sysObj.configureTestCase(ip,port,'MocaHal_Get_SelfNode_TxTotalPackets');
...
if "SUCCESS" in mocaLoadStatus.upper() and sysLoadStatus.upper():
    ...
    if expectedResult in actualResult and ifName:
        ...
        #Get TxPackets using system command
        tdkTestObj = sysObj.createTestStep('ExecuteCommand');
        cmd = "cat /sys/class/net/" + ifName + "/statistics/tx_packets | tr -d '\n'";
        tdkTestObj.addParameter("command", cmd);
        tdkTestObj.executeTestCase("SUCCESS");
        actualResult = tdkTestObj.getResult();
        txPackets = tdkTestObj.getResultDetails();
        print "TxPackets from linux command: ",txPackets

        if expectedResult in actualResult and txPackets:
            tdkTestObj.setResultStatus("SUCCESS")
            #Prmitive test case 
            tdkTestObj = mocahalObj.createTestStep('MocaHal_GetTxTotalPackets');
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            actualResult = tdkTestObj.getResult();
            print "RESULT: GetTxTotalPackets : " , actualResult
            txTotalPackets = tdkTestObj.getResultDetails();
            print "DETAILS: GetTxTotalPackets : " , txTotalPackets

            if expectedResult in actualResult and txTotalPackets:
                print "TxTotalPackets retrieved: ", txTotalPackets
                if txTotalPackets >= txPackets:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print "TxTotalPackets verified"
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print "TxTotalPackets not verified"
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "TxTotalPackets not not retrieved"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Execute command failed to get tx packets"
    else:
        tdkTestObj.setResultStatus("FAILURE")
