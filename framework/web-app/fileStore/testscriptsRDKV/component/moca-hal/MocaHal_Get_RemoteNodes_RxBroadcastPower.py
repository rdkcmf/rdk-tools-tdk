...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_RemoteNodes_RxBroadcastPower</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetRxBroadcastPower</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the RxBroadcastPower of the remote node</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_68</test_case_id>
    <test_objective>To get the RxBroadcastPower of the remote node</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_RemoteNode_GetRxBroadcastPower</api_or_interface_used>
...
[out]	response	The RxBroadcastPower of the remote node.</input_parameters>
...
3. Moca hal agent will invoke the api SoC_IMPL__RMH_RemoteNode_GetRxBroadcastPower with each node id
4. Check the api call return value and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_RemoteNodes_RxBroadcastPower</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_RemoteNodes_RxBroadcastPower');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    if expectedresult in actualresult and remoteNodeList:
        ...
        for nodeId in remoteNodeList:
            #Prmitive test case to get remote node RxBroadcastPower
            tdkTestObj = mocahalObj.createTestStep('MocaHal_RemoteNode_GetRxBroadcastPower');
            tdkTestObj.addParameter("nodeId",int(nodeId));
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            print "RESULT: RemoteNode_GetRxBroadcastPower : " , actualresult
            power = tdkTestObj.getResultDetails();
            print "DETAILS: RemoteNode_GetRxBroadcastPower : " , power

            if expectedresult in actualresult and power:
                tdkTestObj.setResultStatus("SUCCESS")
                print "RxBroadcastPower %s retrieved for node id %d" %(power, int(nodeId))
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "RxBroadcastPower not retrieved for node id ", int(nodeId)
    else:
        tdkTestObj.setResultStatus("FAILURE")
