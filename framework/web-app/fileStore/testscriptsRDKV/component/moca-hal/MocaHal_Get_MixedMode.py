...
<xml>
  ...
  <name>MocaHal_Get_MixedMode</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>MocaHal_GetMixedMode</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the MoCA network contains both 1.1 and 2.0 nodes</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_30</test_case_id>
    <test_objective>To check if the MoCA network contains both 1.1 and 2.0 nodes.</test_objective>
...
[out] response	Set to true if mixedmode is enabled. Otherwise set to false.</input_parameters>
...
3. Veirfy that mixedmode enabled value is retrieved
...
Checkpoint 2 Verify that the mixedmode enabled value is retrieved</expected_output>
    <priority>High</priority>
    <test_stub_interface>libmocahalstub.so</test_stub_interface>
    <test_script>MocaHal_Get_MixedMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_MixedMode');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    tdkTestObj = mocahalObj.createTestStep('MocaHal_GetMixedMode');
    #Execute the test case in STB
    tdkTestObj.executeTestCase(expectedresult);
    actualresult = tdkTestObj.getResult();
    print "RESULT: GetMixedMode : " , actualresult

    if expectedresult in actualresult:
        status = tdkTestObj.getResultDetails();
        print "DETAILS: GetMixedMode : " , status
        if "true" or "false" in status:
            tdkTestObj.setResultStatus("SUCCESS")
            print "MixedMode enabled has valid  value" , status
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "MixedMode enabled has invalid value"
    else:
        tdkTestObj.setResultStatus("FAILURE")
        print "GetMixedMode call failed"
