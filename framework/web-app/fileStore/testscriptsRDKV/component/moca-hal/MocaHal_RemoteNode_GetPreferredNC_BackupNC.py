...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_RemoteNode_GetPreferredNC_BackupNC</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetPreferredNC</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check the preferred NC status of backup network co-ordinator</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_43</test_case_id>
    <test_objective>To check the preferred NC status of backup network co-ordinator</test_objective>
...
[out]	response	Preferred NC status</input_parameters>
    <automation_approch>1. TM loads the Moca hal agent via the test agent.
2 .Moca hal agent will invoke the api SoC_IMPL__RMH_RemoteNode_GetPreferredNC with node id of backup network co-ordinator
3. Check the api call return value as "true" or "false" and return SUCCESS/FAILURE status.</automation_approch>
    <except_output>Checkpoint 1.Verify the API call return value
                   Checkpoint 2.Verify the return value is "true" or "false" </except_output>
    <priority>High</priority>
    <test_stub_interface>libmocahalstub.so</test_stub_interface>
    <test_script>MocaHal_RemoteNode_GetPreferredNC_BackupNC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_RemoteNode_GetPreferredNC_BackupNC');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    print "RESULT: Backup NCNodeId : " , actualresult
    bkNcNodeId = tdkTestObj.getResultDetails();
    print "DETAILS: Backup NCNodeId : " , bkNcNodeId

    if expectedresult in actualresult and bkNcNodeId:
        tdkTestObj.setResultStatus("SUCCESS")
        print "NCNodeId retrieved" , bkNcNodeId
        ...
        if (expectedresult in actualresult) and (status == "true" or status == "false"):
            tdkTestObj.setResultStatus("SUCCESS")
            print "GetPreferredNC status retrieved for backup network co-ordinator"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "GetPreferredNC status not retrieved for backup network co-ordinator"
    else:
        tdkTestObj.setResultStatus("FAILURE")
