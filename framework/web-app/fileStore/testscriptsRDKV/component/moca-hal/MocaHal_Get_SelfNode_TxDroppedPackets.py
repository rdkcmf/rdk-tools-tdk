...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_SelfNode_TxDroppedPackets</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_GetTxDroppedPackets</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the total number of packets dropped during transmission</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_56</test_case_id>
    <test_objective>To get the total number of packets dropped during transmission</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_Stats_GetTxDroppedPackets</api_or_interface_used>
...
2. Using systemutil ExecuteCmd command get the tx dropped packets using linux command
3. Moca hal agent will invoke the api SoC_IMPL__RMH_Stats_GetTxDroppedPackets 
4. Check the value from mocahal interface is greater than or equal to the value from linux command and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_SelfNode_TxDroppedPackets</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_SelfNode_TxDroppedPackets');
sysObj.configureTestCase(ip,port,'MocaHal_Get_SelfNode_TxDroppedPackets');
...
if "SUCCESS" in mocaLoadStatus.upper() and sysLoadStatus.upper():
    ...
    if expectedResult in actualResult and ifName:
        ...
        #Get TxDroppedPackets using system command
        tdkTestObj = sysObj.createTestStep('ExecuteCommand');
        cmd = "cat /sys/class/net/" + ifName + "/statistics/tx_dropped | tr -d '\n'";
        tdkTestObj.addParameter("command", cmd);
        tdkTestObj.executeTestCase("SUCCESS");
        actualResult = tdkTestObj.getResult();
        txPackets = tdkTestObj.getResultDetails();
        print "TxDroppedPackets from linux command: ",txPackets

        if expectedResult in actualResult and txPackets:
            tdkTestObj.setResultStatus("SUCCESS")
            #Prmitive test case 
            tdkTestObj = mocahalObj.createTestStep('MocaHal_GetTxDroppedPackets');
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            actualResult = tdkTestObj.getResult();
            print "RESULT: GetTxDroppedPackets : " , actualResult
            txDroppedPackets = tdkTestObj.getResultDetails();
            print "DETAILS: GetTxDroppedPackets : " , txDroppedPackets

            if expectedResult in actualResult and txDroppedPackets:
                print "TxDroppedPackets retrieved: ", txDroppedPackets
                if txDroppedPackets >= txPackets:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print "TxDroppedPackets verified"
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print "TxDroppedPackets not verified"
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "TxDroppedPackets not retrieved"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Execute command failed to get tx dropped packets"
    else:
        tdkTestObj.setResultStatus("FAILURE")
