...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_RemoteNode_GetPreferredNC_CurrentNC</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetPreferredNC</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check the preferred NC status of current network co-ordinator</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_42</test_case_id>
    <test_objective>To check the preferred NC status of current network co-ordinator</test_objective>
...
2 .Moca hal agent will invoke the api SoC_IMPL__RMH_RemoteNode_GetPreferredNC with node id of current network co-ordinator
3. Check the api call return value as "true" or "false" and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_RemoteNode_GetPreferredNC_CurrentNC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_RemoteNode_GetPreferredNC_CurrentNC');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    if expectedresult in actualresult and ncNodeId:
        ...
        if (expectedresult in actualresult) and status == "true":
            tdkTestObj.setResultStatus("SUCCESS")
            print "GetPreferredNC status retrieved for current network co-ordinator"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "GetPreferredNC status not retrieved for current network co-ordinator"
    else:
        tdkTestObj.setResultStatus("FAILURE")
