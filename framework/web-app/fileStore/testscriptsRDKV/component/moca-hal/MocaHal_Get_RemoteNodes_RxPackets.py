...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_RemoteNodes_RxPackets</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetRxPackets</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the number of packets received by the remote node</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_71</test_case_id>
    <test_objective>To get the number of packets received by the remote node</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_RemoteNode_GetRxPackets</api_or_interface_used>
...
[out]	response	The number of packets received by the remote node.</input_parameters>
...
3. Moca hal agent will invoke the api SoC_IMPL__RMH_RemoteNode_GetRxPackets with each node id
4. Check the api call return value and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_RemoteNodes_RxPackets</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_RemoteNodes_RxPackets');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    if expectedresult in actualresult and remoteNodeList:
        ...
        for nodeId in remoteNodeList:
            #Prmitive test case to get remote node RxPackets
            tdkTestObj = mocahalObj.createTestStep('MocaHal_RemoteNode_GetRxPackets');
            tdkTestObj.addParameter("nodeId",int(nodeId));
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            print "RESULT: RemoteNode_GetRxPackets : " , actualresult
            rxPackets = tdkTestObj.getResultDetails();
            print "DETAILS: RemoteNode_GetRxPackets : " , rxPackets

            if expectedresult in actualresult and rxPackets:
                tdkTestObj.setResultStatus("SUCCESS")
                print "RxPackets %s retrieved for node id %d" %(rxPackets, int(nodeId))
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "RxPackets not retrieved for node id ", int(nodeId)
    else:
        tdkTestObj.setResultStatus("FAILURE")
