...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_RemoteNodes_MacAddress</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetMac</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the mac address of the remote nodes using RemoteNode_GetMac method</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_41</test_case_id>
    <test_objective>To get the mac address of the remote nodes using RemoteNode_GetMac method</test_objective>
...
3. Moca hal agent will invoke the api SoC_IMPL__RMH_RemoteNode_GetMac with each node id
4. Check the api call return value and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_RemoteNodes_MacAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_RemoteNodes_MacAddress');
    ...
    if expectedresult in actualresult and remoteNodeList:
        ...
        for nodeId in remoteNodeList:
            ...
            if expectedresult in actualresult and macAddr:
                tdkTestObj.setResultStatus("SUCCESS")
                print "MacAddress %s retrieved for node id %d" %(macAddr, int(nodeId))
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "MacAddress not retrieved for node id ", int(nodeId)
    else:
        tdkTestObj.setResultStatus("FAILURE")
