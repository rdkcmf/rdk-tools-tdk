...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>MocaHal_Get_RemoteNodes_RxUnicastPhyRate</name>
  <primitive_test_id/>
  <primitive_test_name>MocaHal_RemoteNode_GetRxUnicastPhyRate</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the RxUnicastPhyRate of the remote node</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_MOCA_HAL_70</test_case_id>
    <test_objective>To get the RxUnicastPhyRate of the remote node</test_objective>
...
2.Destroy the moca handle at the end of the test.</pre_requisite>
    <api_or_interface_used>SoC_IMPL__RMH_RemoteNode_GetRxUnicastPhyRate</api_or_interface_used>
...
[out]	response	The RxUnicastPhyRate of the remote node.</input_parameters>
...
3. Moca hal agent will invoke the api SoC_IMPL__RMH_RemoteNode_GetRxUnicastPhyRate with each node id
4. Check the api call return value and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>MocaHal_Get_RemoteNodes_RxUnicastPhyRate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mocahalObj.configureTestCase(ip,port,'MocaHal_Get_RemoteNodes_RxUnicastPhyRate');
...
if "SUCCESS" in mocaLoadStatus.upper():
    ...
    if expectedresult in actualresult and remoteNodeList:
        ...
        for nodeId in remoteNodeList:
            #Prmitive test case to get remote node RxUnicastPhyRate
            tdkTestObj = mocahalObj.createTestStep('MocaHal_RemoteNode_GetRxUnicastPhyRate');
            tdkTestObj.addParameter("nodeId",int(nodeId));
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            print "RESULT: RemoteNode_GetRxUnicastPhyRate : " , actualresult
            phyRate = tdkTestObj.getResultDetails();
            print "DETAILS: RemoteNode_GetRxUnicastPhyRate : " , phyRate

            if expectedresult in actualresult and phyRate:
                tdkTestObj.setResultStatus("SUCCESS")
                print "RxUnicastPhyRate %s retrieved for node id %d" %(phyRate, int(nodeId))
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print "RxUnicastPhyRate not retrieved for node id ", int(nodeId)
    else:
        tdkTestObj.setResultStatus("FAILURE")
