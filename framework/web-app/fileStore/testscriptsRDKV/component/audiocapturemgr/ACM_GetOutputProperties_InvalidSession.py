...
<xml>
  ...
  <name>ACM_GetOutputProperties_InvalidSession</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify that output properties not retrieved for invalid ACM session</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_ACM_07</test_case_id>
    <test_objective>To verify that output properties not retrieved for invalid ACM session</test_objective>
...
3. AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_GET_OUTPUT_PROPS with invalid session id.
...
Checkpoint 2.Verify the IARMBUS call for IARMBUS_AUDIOCAPTUREMGR_GET_OUTPUT_PROPS is FAILURE</except_output>
    <priority>High</priority>
    <test_stub_interface>libaudiocapturemgrstub.so</test_stub_interface>
    <test_script>ACM_GetOutputProperties_InvalidSession</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'ACM_GetOutputProperties_InvalidSession');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        acmObj.configureTestCase(ip,port,'ACM_GetOutputProperties_InvalidSession');
					...
					if sessionId == -1:
						...
						tdkTestObj.addParameter("session", "INVALID");
                          			...
                          			#Set the result status of execution
                                		if expectedresult in actualresult:
                                		        tdkTestObj.setResultStatus("SUCCESS");
                                                	print "Output Properties not retrieved for invalid session id";
						else:
                                                	tdkTestObj.setResultStatus("FAILURE");
                                                	print "Output Properties retrieved for invalid session id";
