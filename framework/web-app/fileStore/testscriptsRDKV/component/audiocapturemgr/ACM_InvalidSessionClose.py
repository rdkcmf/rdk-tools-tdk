...
<xml>
  ...
  <name>ACM_InvalidSessionClose</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify that ACM session cannot be closed without opening session</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_ACM_11</test_case_id>
    <test_objective>To verify that ACM session cannot be closed without opening session</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Xg1v3</test_setup>
    <pre_requisite>None</pre_requisite>
    <api_or_interface_used>IARM_Bus_Call(IARMBUS_AUDIOCAPTUREMGR_NAME, IARMBUS_AUDIOCAPTUREMGR_CLOSE, (void *) &amp;param, sizeof(param))</api_or_interface_used>
...
2. Without opening an ACM session AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_CLOSE
3. Check if the IARMBUS call is failure and return SUCCESS/FAILURE</automation_approch>
    <except_output>Checkpoint 1.Verify the IARMBUS call is FAILURE</except_output>
    <priority>High</priority>
    <test_stub_interface>libaudiocapturemgrstub.so</test_stub_interface>
    <test_script>ACM_InvalidSessionClose</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'ACM_InvalidSessionClose');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        acmObj.configureTestCase(ip,port,'ACM_InvalidSessionClose');
					...
					print "AudioCaptureMgr session could not be closed as it is not available";
                                else:
                                        tdkTestObj.setResultStatus("FAILURE");
					print "AudioCaptureMgr session closed even though it is not available";
