...
<xml>
  ...
  <name>ACM_GetDefaultAudioProperties_InvalidSession</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify that default audio properties not retrieved for invalid ACM session</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_ACM_03</test_case_id>
    <test_objective>To verify that default audio properties not retrieved for invalid ACM session</test_objective>
...
3. AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_GET_DEFAULT_AUDIO_PROPS with invalid session id.
...
Checkpoint 2.Verify the IARMBUS call for IARMBUS_AUDIOCAPTUREMGR_GET_DEFAULT_AUDIO_PROPS is FAILURE</except_output>
    <priority>High</priority>
    <test_stub_interface>libaudiocapturemgrstub.so</test_stub_interface>
    <test_script>ACM_GetDefaultAudioProperties_InvalidSession</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'ACM_GetDefaultAudioProperties_InvalidSession');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        acmObj.configureTestCase(ip,port,'ACM_GetDefaultAudioProperties_InvalidSession');
					...
					if sessionId == -1:
						...
						tdkTestObj.addParameter("session", "INVALID");
                          			...
                          			#Set the result status of execution
                                		if expectedresult in actualresult:
                                		        tdkTestObj.setResultStatus("SUCCESS");
                                                	print "Default Audio Properties not retrieved for invalid session id";
						else:
                                                	tdkTestObj.setResultStatus("FAILURE");
                                                	print "Default Audio Properties retrieved for invalid session id";
