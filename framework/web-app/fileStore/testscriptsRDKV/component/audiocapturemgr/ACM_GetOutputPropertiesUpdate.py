...
<xml>
  ...
  <name>ACM_GetOutputPropertiesUpdate</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify output properties changes over session</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_ACM_13</test_case_id>
    <test_objective>To verify output properties changes over session</test_objective>
...
IARM_Bus_Call(IARMBUS_AUDIOCAPTUREMGR_NAME, IARMBUS_AUDIOCAPTUREMGR_GET_OUTPUT_PROPS, (void *) &amp;param, sizeof(param))
...
4. Check if the IARMBUS call is success and the audio properties returned is non empty.
5. AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_CLOSE
6. Repeat steps 2-3 and verify that new socket path retrieved is different and return SUCCESS/FAILURE
7. AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_CLOSE</automation_approch>
    <except_output>Checkpoint 1.Verify the IARMBUS calls are SUCCESS 
Checkpoint 2.Verify the value returned for output properties changes over session</except_output>
    <priority>High</priority>
    <test_stub_interface>libaudiocapturemgrstub.so</test_stub_interface>
    <test_script>ACM_GetOutputPropertiesUpdate</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'ACM_GetOutputPropertiesUpdate');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        acmObj.configureTestCase(ip,port,'ACM_GetOutputPropertiesUpdate');
					...
					if sessionId == -1:
						...
						#Prmitive test case which associated to this Script
                          			...
                          			#Set the result status of execution
                                		if expectedresult in actualresult:
                                        		socketPath = tdkTestObj.getResultDetails();
                                        		print "Socket Path: %s"%socketPath;
							if socketPath != "":
                                		        	tdkTestObj.setResultStatus("SUCCESS");
	                                                	print "Socket Path retrieved";
							else:
                                                		tdkTestObj.setResultStatus("FAILURE");
			                                        print "Socket Path retrieved is empty";
						else:
                                                	tdkTestObj.setResultStatus("FAILURE");
						...
						#Prmitive test case which associated to this Script
		                                ...
		                                #Set the result status of execution
                       			        if expectedresult in actualresult:
                                               		...
                                               		print "AudioCaptureMgr session closed successfully";
                                                        ...
                                                        if expectedresult in actualresult:
								sessionId = tdkTestObj.getResultDetails();
                			                        print "New Session ID: %s"%sessionId;
								...
								#Set the result status of execution
								if expectedresult in actualresult:
									newSocketPath = tdkTestObj.getResultDetails();
									print "New Socket Path: %s"%newSocketPath;
	
									if newSocketPath !=socketPath:
                                                                                tdkTestObj.setResultStatus("SUCCESS");
                                                                                print "Socket path changes over session";
                                                                        else:
                                                                                tdkTestObj.setResultStatus("FAILURE");
                                                                                print "Socket does not change over session";
