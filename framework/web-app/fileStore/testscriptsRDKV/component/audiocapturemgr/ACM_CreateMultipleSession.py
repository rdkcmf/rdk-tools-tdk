...
<xml>
  ...
  <name>ACM_CreateMultipleSession</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if session id changes for different ACM sessions</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_ACM_12</test_case_id>
    <test_objective>To check if session id changes for different ACM sessions</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Xg1v3</test_setup>
    <pre_requisite>None</pre_requisite>
    <api_or_interface_used>IARM_Bus_Call(IARMBUS_AUDIOCAPTUREMGR_NAME, IARMBUS_AUDIOCAPTUREMGR_OPEN, (void *) &amp;param, sizeof(param))</api_or_interface_used>
...
3. AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_CLOSE
4. AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_OPEN again and verify that the session id obtained is different from the session id obtained in step 2 and return SUCCESS/FAILURE</automation_approch>
    <except_output>Checkpoint 1.Verify the IARMBUS calls are SUCCESS
Checkpoint 2.Verify that different session ids are obtained for each session opened</except_output>
    <priority>High</priority>
    <test_stub_interface>libaudiocapturemgrstub.so</test_stub_interface>
    <test_script>ACM_CreateMultipleSession</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'ACM_CreateMultipleSession');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        acmObj.configureTestCase(ip,port,'ACM_CreateMultipleSession');
					...
					if sessionId == -1:
						...
						#Prmitive test case which associated to this Script
							...
							#Set the result status of execution
							if expectedresult in actualresult:
								newSessionId = tdkTestObj.getResultDetails();
								print "New Session ID: %s"%newSessionId;
								if newSessionId == -1:
									tdkTestObj.setResultStatus("FAILURE");
									print "Failed to open AudioCaptureMgr session";
								else:
									if sessionId != newSessionId:
										tdkTestObj.setResultStatus("SUCCESS");
										print "New ACM session opened with different session id";
										#Prmitive test case which associated to this Script
