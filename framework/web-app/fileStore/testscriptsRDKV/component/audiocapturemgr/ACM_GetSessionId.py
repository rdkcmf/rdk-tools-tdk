...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify that ACM session is is retrieved correctly once the session is opened.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_ACM_10</test_case_id>
    <test_objective>To verify that ACM session is is retrieved correctly once the session is opened.</test_objective>
...
3. AudioCaptureMgr agent will retrieve the session id.
4. Check if the session is retrieved is same as the session id created by IARMBUS_AUDIOCAPTUREMGR_OPEN  and return SUCCESS/FAILURE
...
Checkpoint 2.Verify the session id retrieved is same as that of the open session</except_output>
    <priority>High</priority>
    <test_stub_interface>libaudiocapturemgrstub.so</test_stub_interface>
    <test_script>ACM_GetSessionId</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'ACM_GetSessionId');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        acmObj.configureTestCase(ip,port,'ACM_GetSessionId');
					...
					if sessionId == -1:
						...
						#Prmitive test case which associated to this Script
		                                tdkTestObj = acmObj.createTestStep('AudioCaptureMgr_GetSessionDetails');
                          			...
                          			#Set the result status of execution
                                		if expectedresult in actualresult:
                                        		sessionId1 = tdkTestObj.getResultDetails();
                                        		print "Session ID: %s"%sessionId1;
                                        		if sessionId1 == sessionId:
                                		                tdkTestObj.setResultStatus("SUCCESS");
                                                		print "AudioCaptureMgr session id retrieved correctly";
                		                        else:
                                                		tdkTestObj.setResultStatus("FAILURE");
		                                                print "AudioCaptureMgr session id retrieved is not correct";
						else:
                                                	tdkTestObj.setResultStatus("FAILURE");
		                                        print "Failed to get AudioCaptureMgr session id";
