...
<xml>
  ...
  <name>ACM_GetDefaultAudioProperties</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve default audio properties for ACM</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_ACM_02</test_case_id>
    <test_objective>To retrieve default audio properties for ACM</test_objective>
...
IARM_Bus_Call(IARMBUS_AUDIOCAPTUREMGR_NAME, IARMBUS_AUDIOCAPTUREMGR_GET_DEFAULT_AUDIO_PROPS, (void *) &amp;param, sizeof(param))
...
3. AudioCaptureMgr agent will invoke the IARM_Bus_Call for IARMBUS_AUDIOCAPTUREMGR_GET_DEFAULT_AUDIO_PROPS
4. Check if the IARMBUS call is success and the default audio properties returned is non empty and return SUCCESS/FAILURE
...
Checkpoint 2.Verify the value returned for default audio properties is not empty</except_output>
    <priority>High</priority>
    <test_stub_interface>libaudiocapturemgrstub.so</test_stub_interface>
    <test_script>ACM_GetDefaultAudioProperties</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'ACM_GetDefaultAudioProperties');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        acmObj.configureTestCase(ip,port,'ACM_GetDefaultAudioProperties');
					...
					if sessionId == -1:
						...
						#Prmitive test case which associated to this Script
		                                tdkTestObj = acmObj.createTestStep('AudioCaptureMgr_GetDefaultAudioProperties');
							...
							properties = ast.literal_eval(tdkTestObj.getResultDetails());
                                                        print "Default Audio Properties: %s"%properties;
                                                        if (properties['DelayComp'] != "" or properties['Threshold'] != "" or propertiess['FifoSize'] != ""):
                                                                tdkTestObj.setResultStatus("SUCCESS");
                                                                print "Default Audio Properties retrieved";
                                                        else:
                                                                tdkTestObj.setResultStatus("FAILURE");
                                                                print "Retrieved Default Audio Properties not correct";
						else:
                                                	tdkTestObj.setResultStatus("FAILURE");
		                                        print "Failed to retrieve Default Audio Properties";
