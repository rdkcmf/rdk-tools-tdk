...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1330</id>
  <version>3</version>
  <name>TR069_Get_DeviceProcessorArchitecture_25</name>
  ...
  <status>FREE</status>
  <synopsis>Objective: To fetch the type of processor acrhitecture used by querying  the tr69Hostif through curl.  Query string "Device.DeviceInfo.Processor.1.Architecture". 
TestCaseID: CT_TR69_25
TestType: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_TR69_25</test_case_id>
    <test_objective>To fetch the type of processor acrhitecture used by querying 
the tr69Hostif through curl. 
Query string "Device.DeviceInfo.Processor.1.Architecture".
No set operation avaliable for this parameter.
Assumption every device should atleast have 1 processor.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3</test_setup>
    <pre_requisite>No</pre_requisite>
    <api_or_interface_used>curl -d '{"paramList" : [{"name" : "Device.DeviceInfo.Processor.1.Architecture"}]}' http://127.0.0.1:10999</api_or_interface_used>
...
"Device.DeviceInfo.Processor.1.Architecture" to fetch the type of  processor architecture.
3. Tr69Test agent will get the curl response which be a vaild string  on SUCCESS.
4. If tr69Test agent will get the empty curl response if FAILURE.
5. TM Unloads tr69Test agent.</automation_approch>
    <except_output>Checkpoint 1. Need to get valid string on SUCCESS. Empty on FAILURE.
Checkpoint 2: We can use uname() systemcall to verify correctness of value.</except_output>
    <priority>High</priority>
    <test_stub_interface>libtr69stub.so</test_stub_interface>
    <test_script>TR069_Get_DeviceProcessorArchitecture_25</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TR069_Get_DeviceProcessorArchitecture_25');
...
profilePath = "Device.DeviceInfo.Processor.1.Architecture"
