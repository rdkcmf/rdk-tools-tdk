...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1310</id>
  <version>2</version>
  <name>TR069_Get_DeviceFirstUseDate_12</name>
  ...
  <status>FREE</status>
  <synopsis>Objective: To fetch the device first use date by querying  the tr69Hostif through curl.  Query string "Device.DeviceInfo.FirstUseDate". No set operation avaliable for this parameter.
TestCaseID: CT_TR69_12
TestType: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_TR69_12</test_case_id>
    <test_objective>To fetch the device first use date by querying 
the tr69Hostif through curl. 
Query string "Device.DeviceInfo.FirstUseDate".
No set operation avaliable for this parameter.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3</test_setup>
    <pre_requisite>No</pre_requisite>
    <api_or_interface_used>curl -d '{"paramList" : [{"name" : "Device.DeviceInfo.FirstUseDate"}]}' http://127.0.0.1:10999</api_or_interface_used>
...
"Device.DeviceInfo.FirstUseTime" to fetch the date and time when CPE successfully established connection with IP Network layer.
3. Tr69Test agent will get the curl response which be a vaild date and time  on SUCCESS.
4. If tr69Test agent will get the empty curl response if FAILURE.
5. TM Unloads tr69Test agent.</automation_approch>
    <except_output>Checkpoint 1. Need to return a valid date and time on SUCCESS. Empty on FAILURE.</except_output>
    <priority>High</priority>
    <test_stub_interface>libtr69stub.so</test_stub_interface>
    <test_script>TR069_Get_DeviceFirstUseDate_12</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TR069_Get_DeviceFirstUseDate_12');
...
profilePath = "Device.DeviceInfo.FirstUseDate"
