...
TestType: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_TR69_30</test_case_id>
    <test_objective>To fetch  the status (enabled/disabled) of the IPv4 stack on a device by querying tr69Hostif through curl. 
Query string "Device.IP.IPv4Enable".</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3</test_setup>
    <pre_requisite>No</pre_requisite>
    <api_or_interface_used>curl -d '{"paramList" : [{"name" : "Device.IP.IPv4Enable"}]}' http://127.0.0.1:10999</api_or_interface_used>
...
"Device.IP.IPv4Enable." to fetch the status (enabled/disabled) of the IPv4 stack on a device.
3. Tr69Test agent will get the curl response which be a vaild string “Enabled” or “Disabled” on SUCCESS.
4. If tr69Test agent will get the empty curl response if FAILURE.
5. TM Unloads tr69Test agent.</automation_approch>
    <except_output>Checkpoint 1. Need to get valid string “true” or “false” on SUCCESS. Empty on FAILURE.
Checkpoint 2. We can cross check the value using ifconfig.</except_output>
    ...
    <test_script>TR069_Get_DeviceIPv4Enable_30</test_script>
    <skipped>No</skipped>
