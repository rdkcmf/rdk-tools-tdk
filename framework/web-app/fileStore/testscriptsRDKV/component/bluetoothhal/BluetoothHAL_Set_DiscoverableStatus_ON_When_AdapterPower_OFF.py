...
<xml>
  ...
  <name>BluetoothHAL_Set_DiscoverableStatus_ON_When_AdapterPower_OFF</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify that discoverable status cannot be changed to 1 (ON) when adapter power is 0 (OFF)</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_18</test_case_id>
    <test_objective>To verify that discoverable status cannot be changed to 1 (ON) when adapter power is 0 (OFF)</test_objective>
...
4. enBTRCoreRet BTRCore_SetAdapterDiscoverable (tBTRCoreHandle hBTRCore, const char* pAdapterPath, unsigned char discoverable);</api_or_interface_used>
    <input_parameters>1. adapter_path - retrieved using BTRCore_GetAdapter()
2. power_status - 0 (OFF)
3. discoverable_status - 1 , to set the device as discoverable</input_parameters>
...
3. BluetoothHal agent will invoke the api BTRCore_SetAdapterPower to set the power status of bluetooth adapter to 0 (OFF).
4. Read back the power value with BTRCore_GetAdapterPower() again and verify that the adapter is powered OFF correctly.
5. BluetoothHal agent will invoke the api BTRCore_SetAdapterDiscoverable to set the discoverable status for the adapter as 1(discoverable)
6. Bluetooth adapter power is reset to 1 (ON).
7. Based on the API call return code, TM return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1. Verify the API call is success
Checkpoint 2. Verify that the discoverable status for bluetooth adapter must not be set to 1, when adapter power is 0 (OFF).</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Set_DiscoverableStatus_ON_When_AdapterPower_OFF</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Set_DiscoverableStatus_ON_When_AdapterPower_OFF');
		...
		if (actualresult == expectedresult):
			...
			tdkTestObj.setResultStatus("SUCCESS");
		        ...
		        if (actualresult == expectedresult):
	                    ...
	                    print "Discoverable status is changed to 1 when adapter power status is OFF"
                        else:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "Discoverable status could not be changed when adapter power status is OFF"
                        #Reset the bluetooth adapter power to ON state
                        print "Resetting bluetooth adapter power back to ON"
