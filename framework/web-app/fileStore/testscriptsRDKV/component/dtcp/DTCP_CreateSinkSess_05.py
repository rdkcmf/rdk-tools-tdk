...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>23</version>
  <name>DTCP_CreateSinkSess_05</name>
  <primitive_test_id/>
  <primitive_test_name>DTCP_Comp_Test</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>To create a new authenticated session with a remote DTCP-IP source.
TestType: Positive		
TestcaseID: CT_DTCP_05</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_DTCP_05</test_case_id>
    <test_objective>To create a new authenticated session with a remote DTCP-IP source.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG1</test_setup>
    <pre_requisite>DTCPMgrInitialize, DTCPMgrStartSource</pre_requisite>
    <api_or_interface_used>dtcp_result_t DTCPMgrCreateSinkSession(char *srcIpAddress, int srcIpPort, BOOLEAN uniqueKey, int maxPacketSize, DTCP_SESSION_HANDLE *handle);</api_or_interface_used>
    <input_parameters>'srcIp':lan0Ip,'srcPort':5000,'uniqueKey':0,'maxPacketSize':4096</input_parameters>
    <automation_approch>1.TM loads DTCP_agent via the test agent. 
2.The stub will invokes the RPC method for to create sink session.
...
4. TM will receive and display the result.</automation_approch>
    ...
    <test_script>DTCP_CreateSinkSess_05</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DTCP_CreateSinkSess_05');
...
if "SUCCESS" in loadmodulestatus.upper():
  ...
  dtcp.startSource(tdkTestObj,expectedresult,kwargs={'ifName':'lo','port':5005})
