...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <status>FREE</status>
  <synopsis>This tests validate tuner reservation request on a channel that is being recorded.
Test Case ID: CT_TRM_28
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_TRM_28</test_case_id>
    <test_objective>To request to validate tuner reservation on a channel that is being recorded</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG1-1</test_setup>
    <pre_requisite>None</pre_requisite>
    <api_or_interface_used>ReserveTuner
ValidateTunerReservation</api_or_interface_used>
...
2. TM will invoke first invoke “TRMAgent_TunerReserveForRecord” and then “TRMAgent_ValidateTunerReservation” in TRMAgent. 
3. TRMAgent will connect to TRM Server on IP 127.0.0.1 port 9987 and post HTTP TRM ReserveTuner for record request followed by ValidateTunerReservation request message after waiting for 2 sec from device 1 on channel 1.
...
TestMgr_TRM_ValidateTunerReservation</test_stub_interface>
    <test_script>TRM_CT_28</test_script>
    <skipped>No</skipped>
...
import tdklib;
import time;
from trm import validateReservation,reserveForRecord
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TRM_CT_28');
...
if "SUCCESS" in result.upper():
    ...
    validateReservation(obj,"SUCCESS",kwargs={'deviceNo':deviceNo,'activity':2,'streamId':streamId})
