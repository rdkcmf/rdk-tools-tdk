...
<xml>
  ...
  <name>FCS_Playback_HLS_480p</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do HLS Videoplayback with resolution set to 480p through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To test the video playback of HLS with resolution set to 480p through 'playbin' and 'westerossink' gst elements</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_HLS_480p</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_HLS_480p')
dsObj.configureTestCase(ip,port,'FCS_Playback_HLS_480p')
