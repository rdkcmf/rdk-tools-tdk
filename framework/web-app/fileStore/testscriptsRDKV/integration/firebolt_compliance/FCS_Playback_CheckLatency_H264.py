...
2. Test stream url for a live H264 stream should be updated in the config variable video_src_url_hls_h264 inside MediaValidationVariables.py library inside filestore.
...
test_url - h264 live stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hls_h264)
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_latency <H264_STREAM_URL> checkavstatus=yes timeout=20 latencyThreshold=100"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "Latency retrieved was optimal for H264 playback"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Latency retrieved was not optimal for H264 playback"
    else:
        tdkTestObj.setResultStatus("FAILURE")
