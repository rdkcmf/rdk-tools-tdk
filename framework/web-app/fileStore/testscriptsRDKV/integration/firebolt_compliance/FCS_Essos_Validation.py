...
import tdklib;
import FCS_EssosValidation_utility
...
if "SUCCESS" in sysutilLoadStatus.upper():
    ...
    logFile,EssosValidation_log = FCS_EssosValidation_utility.getLogFile(obj);
    ...
    options = options + " -t=7"
    details = FCS_EssosValidation_utility.RunTest(obj,Test,logFile,options);
    ...
    try:
        FCS_EssosValidation_utility.PrintTitle("SUMMARY OF TEST EXECUTION")
        FCS_EssosValidation_utility.Summary(filepath);
        ...
        Failures = FCS_EssosValidation_utility.getNumberOfFailures(filepath)
        if Failures:
    ...
    FCS_EssosValidation_utility.deleteLogFile(obj,EssosValidation_log,EssosValidationExecutionStatus);
