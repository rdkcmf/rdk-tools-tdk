...
<xml>
  ...
  <name>FCS_Playback_FastForward_16x_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do fast forward of a HLS stream with playback rate 16</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test to do fast forward of a HLS stream with playback rate 16</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_FastForward_16x_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_FastForward_16x_HLS');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <HLS_STREAM_URL> checkavstatus=yes operations=fastforward16x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "Fastforward on HLS stream with 16x speed was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Fastforward on HLS stream with 16x speed failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
