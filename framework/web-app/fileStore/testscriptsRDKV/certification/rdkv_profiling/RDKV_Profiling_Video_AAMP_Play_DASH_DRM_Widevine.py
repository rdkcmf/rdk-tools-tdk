...
<xml>
  ...
  <name>RDKV_Profiling_Video_AAMP_Play_DASH_DRM_Widevine</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate profiling data after performing video playback of dash DRM Widevine protected content using lightning uve aamp player app</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PROFILING_16</test_case_id>
    <test_objective>The objective of this test is to validate profiling data after performing video playback of dash DRM Widevine protected content using lightning uve aamp player app</test_objective>
...
2. Set the URL of lightning uve aamp test app with Widevine protected video src url, drm license url.
...
8. Revert the Webkit instance URL and plugin status.</automation_approch>
    ...
    <test_script>RDKV_Profiling_Video_AAMP_Play_DASH_DRM_Widevine</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Profiling_Video_AAMP_Play_DASH_DRM_Widevine');
...
if expectedResult in result.upper():
    status = "SUCCESS"
    is_front = False
    process_list = ['WPEFramework','WPEWebProcess','WPENetworkProcess','OCDM','tr69hostif']
    conf_file,result2 = get_configfile_name(obj)
    time.sleep(30)
    #Validate system wide profiling data before playing AAMP video Dash Drm Widevine
    end_datetime_string = str(datetime.utcnow()).split('.')[0]
    print "\n Validating system wide profiling mettrics from grafana before playing AAMP video Dash Drm Widevine\n"
    ...
    #Validate process wise profiling data before playing AAMP video Dash Drm Widevine
    print "\n Validating process wise profiling metrics from grafana before playing AAMP video Dash Drm Widevine\n"
    ...
    if expectedResult in result and expectedResult in config_status:
        ...
        if is_front:
            ...
            videoURL  = video_src_url_widevine_dash_h264
            ...
            setURLArgument("drmconfigs",video_src_url_widevine_dash_h264_drmconfigs)
            ...
            if expectedResult in result:
                ...
                if url_loaded_status:
                    ...
                    if video_play_status:
                        ...
                        for result,validation_result,process_wise_methods_list,tdkTestObj in get_processwisemethods(obj,process_list,conf_file):
                            if expectedResult in (result and validation_result):
