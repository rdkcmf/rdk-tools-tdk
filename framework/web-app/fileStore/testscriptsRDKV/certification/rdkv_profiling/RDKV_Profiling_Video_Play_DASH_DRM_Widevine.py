...
<xml>
  ...
  <name>RDKV_Profiling_Video_Play_DASH_DRM_Widevine</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate profiling data after performing video playback of dash widevine DRM protected content using lightning video player app (DASHJS)</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PROFILING_13</test_case_id>
    <test_objective>The objective of this test is to validate profiling data after performing video playback of dash widevine DRM protected content using lightning video player app (DASHJS)</test_objective>
...
2. Set the URL of lightning video test app with video src url, drm license url.
3. Check whether URL is loaded properly.
4. Check video playback by decoder proc validation if applicable.
5. After few minutes, validate the profiling data from Grafana tool with the thredhold values for the pre-configured process list.
...
8. Revert the Webkit instance URL and plugin status.</automation_approch>
    ...
    <test_script>RDKV_Profiling_Video_Play_DASH_DRM_Widevine</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Profiling_Video_Play_DASH_DRM_Widevine');
...
if expectedResult in result.upper():
    ...
    time.sleep(30)
    #Validate system wide profiling data before playing dash drm widevine video
    end_datetime_string = str(datetime.utcnow()).split('.')[0]
    print "\n Validating system wide profiling mettrics from grafana before playing dash drm widevine video\n"
    ...
    #Validate process wise profiling data before playing dash drm widevine video
    print "\n Validating process wise profiling metrics from grafana before playing dash drm widevine video \n"
    ...
    if expectedResult in result and expectedResult in config_status:
        ...
        if is_front:
            ...
            appURL    = lightning_video_test_app_url
