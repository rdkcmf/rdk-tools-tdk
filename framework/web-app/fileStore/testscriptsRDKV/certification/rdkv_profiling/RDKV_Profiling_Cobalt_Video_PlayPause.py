...
5. Check whether video is paused and verify with decoder proc entries if applicable
6. Validate the profiling data from Grafana tool with the threshold values for the pre-configured process list.
7. Execute the smem tool and collect the log
8. Execute pmap tool for the list of given process and collect the log
9. Check for alerts from Grafana tool.
10. Revert the plugin status.</automation_approch>
...
if expectedResult in result.upper():
    ...
    if status == "SUCCESS" and validation_dict != {} and cobalt_test_url != "":
        ...
        time.sleep(30)
        #Validate system wide profiling data before cobalt play pause videoplayback
        end_datetime_string = str(datetime.utcnow()).split('.')[0]
        print "\n Validating system wide profiling mettrics from grafana before cobalt play pause videoplayback \n"
        ...
        #Validate process wise profiling data before cobalt play pause videoplayback
        print "\n Validating process wise profiling metrics from grafana before cobalt play pause videoplayback\n"
        ...
        if cobalt_launch_status in expectedResult:
            ...
            if cobalt_status == 'resumed' and expectedResult in result:
                ...
                if(cobalt_result == expectedResult):
                    tdkTestObj.setResultStatus("SUCCESS")
                    #wait for some time for the screen asking for "who is watching" to appear
                    time.sleep(20)
