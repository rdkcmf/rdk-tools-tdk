...
<xml>
  ...
  <name>RDKV_Profiling_Video_Play_DASH_Clear</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate profiling data after performing video playback of dash clear content using lightning video player app (DASHJS)</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PROFILING_17</test_case_id>
    <test_objective>The objective of this test is to validate profiling data after performing video playback of dash clear content using lightning video player app (DASHJS)</test_objective>
...
8. Revert the Webkit instance URL and plugin status.</automation_approch>
    ...
    <test_script>RDKV_Profiling_Video_Play_DASH_Clear</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Profiling_Video_Play_DASH_Clear');
...
if expectedResult in result.upper():
    ...
    time.sleep(30)
    #Validate system wide profiling data before playing dash clear video
    end_datetime_string = str(datetime.utcnow()).split('.')[0]
    print "\n Validating system wide profiling mettrics from grafana before playing dash clear video \n"
    ...
    #Validate process wise profiling data before playing dash clear video
    print "\n Validating process wise profiling metrics from grafana before playing dash clear video \n"
