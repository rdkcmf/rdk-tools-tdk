...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_PACS_AmazonPrime_ResourceUsage_VideoPlayback</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_setValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to play video in Amazon prime and get CPU load and memory usage.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_42</test_case_id>
    <test_objective>The objective of this test is to play video in Amazon prime and get CPU load and memory usage.</test_objective>
...
6. Validate the video playback
7. Validate CPU load and memory usage</automation_approch>
    <expected_output>Video should be played and CPU load and memory usage must be within the expected limit.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_performance</test_stub_interface>
    <test_script>RDKV_CERT_PACS_AmazonPrime_ResourceUsage_VideoPlayback</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PACS_Amazon_ResourceUsage_VideoPlayback');
                        ...
                        if expectedResult in amazon_result:
                            ...
                            if expectedResult in result:
                                ...
                                if validation_dict["validation_required"]:
                                    ...
                                    print "\n User opted for skipping Video validation \n"
                                if result_val == "SUCCESS":
