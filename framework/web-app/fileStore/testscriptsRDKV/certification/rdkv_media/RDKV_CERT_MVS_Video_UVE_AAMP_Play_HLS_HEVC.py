...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HLS_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of HEVC codec hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of HEVC codec hls stream for few minutes and close the player</test_objective>
...
4. App starts playing the hevc hls stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HLS_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_HLS_HEVC')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_hls_hevc
        ...
        #url=<video_hevc_url>.m3u8&operations=close(60)&autotest=true
