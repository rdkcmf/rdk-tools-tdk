...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_FF16X_Play_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 16x speed for few seconds, continue video play of dash h264 video codec content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 16x speed for few seconds, continue video play of dash h264 video codec content and close the player</test_objective>
...
3. Launch webkit instance with video test app with the src url, operations to be performed, fastforward 16x and playnow 1x with interval.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_FF16X_Play_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_FF16X_Play_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url>.mpd&operations=fastfwd16x(30),playnow(10),close(30)&options=checkInterval(5)&autotest=true&type=dash
