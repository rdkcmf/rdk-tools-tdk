...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_Play_HLS_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE player application via Webkit Browser and perform video play operation of HEVC codec hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning UVE player application via Webkit Browser and perform video play operation of HEVC codec hls stream for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_Play_HLS_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_Play_HLS_HEVC')
