...
##########################################################################
SCRIPT_AT_RDM_DLPATH=/media/apps/TDK/var/TDK/StartTDK.sh
SCRIPT_AT_TMP_DLPATH=/tmp/TDK/var/TDK/StartTDK.sh
...
check_tdk_download()
{
  rdm_status=`cat /opt/persistent/rdmDownloadInfo.txt | grep "TDK"  | awk {'print $5'}`
  log_msg $rdm_status
  if [[ "$rdm_status" == "SUCCESS" ]]; then
      ...
      timestamp_rdm_tdkv_package=`cat /opt/persistent/rdmDownloadInfo.txt | grep "TDK" | awk {'print $2'} | sed 's/.*_\([0-9]\{8,\}\).*/\1/'`
      if [[ "$timestamp_image" == "$timestamp_rdm_tdkv_package" ]]; then
          ...
          if [ ! -z $start_script ]; then
              log_msg "TDK RDM package is downloaded successfully and also startup script is found..."
              return 1
          else
              log_msg "TDK RDM package download status is success but startup script is not present in the package..."
              log_msg $start_script
              return 0
          fi
      else
      ...
      log_msg "TDK RDM Download is failed..."
      return 0
  fi
  
  log_msg "TDK package is not downloaded.."
  return 0
}
...
is_tdk_as_rdm()
{
  ...
  if [[ "$package" == "TDK" ]]; then
    log_msg "TDK RDM package is available for download"
    return 1
  else
    log_msg "TDK RDM package is not available.. it is traditional TDK build"
    return 0
  fi
}
